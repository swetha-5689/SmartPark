...
class ParkingSpot extends React.Component {
  ...
  getSpots() {
    return this.state.data.map((response, i) => {
      if (parseInt(this.state.data[i].spotNumber) % 10 !== 1) {
      return (
        <>
        {!this.state.data[i].isVacant && !this.state.data[i].isReserved &&
        <OverlayTrigger obj = {response} key = {i}
          ...
          overlay={
            <Tooltip>
              ...
              Premium: {this.state.data[i].isPremium.toString()}
            </Tooltip>
          }
          >
            <td>
        ...
        {this.state.data[i].isReserved && !this.state.data[i].isVacant &&
        <OverlayTrigger obj = {response} key = {i}
          placement='top'
          overlay={
            <Tooltip>
